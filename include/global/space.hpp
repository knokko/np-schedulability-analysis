		...
		private:
				...
				Response_times& r, const Node &n, const Job<Time>& j, Interval<Time> range)
			{
				...
				if (j.exceeds_deadline(range.upto())) {
					...
					std::cout << "missed deadline of job " << j << " with range " << range << std::endl;
					...
					if (j.get_deadline() < earliest) {
						if (unfinished(new_n, j)) {
							...
							if (early_exit) 
							{
								...
								Node& next =
									new_node(new_n, j, j.get_job_index(), 0, 0, 0, nullptr); // Maybe give attachment
			...
			void make_initial_node(unsigned num_cores)
			{
				...
				// TODO Test what happens if the first job has a prec
				...
				const Interval<Time> start_times, const Interval<Time> finish_times, const unsigned int ncores)
			{
				...
				Node& next_node = new_node_at(acc, n, j, j.get_job_index(),
					...
					earliest_certain_sequential_source_job_release(n, j), nullptr);
#else
...
//						else ehm;
				...
				//if (dispatched_one) std::cout << "Dispatched " << j << std::endl;
				//std::cout << "did dispatch?" << dispatched_one << std::endl;
			...
			void explore(const Node& n)
			{
					...
					it++)
				{
					...
					//std::cout << "Mid consider " << j << std::endl;
